...
REFRESH_TOKEN = "bb37cf1b3734b67ceff7dde1adacb230be2ed4f98fe37344792b6f0a0e319086e0ecf8f0bf01a038a2aa51ce883d6c3964109ebbcc744581fd653f3d2aa7add0"

_access_token = [None]
_token_expiry = [0]

def get_token():
    if _access_token[0] and time.time() < _token_expiry[0] - 60:
    ...
    try:
        ...
        resp = urllib.request.urlopen(urllib.request.Request(
            HA_URL + "/auth/token", data=body,
            headers={"Content-Type": "application/x-www-form-urlencoded"}
        ))
        result = json.loads(resp.read())
        _access_token[0] = result["access_token"]
        _token_expiry[0] = time.time() + result.get("expires_in", 1800)
        print("Token refreshed OK", flush=True, file=sys.stderr)
...
def ha_post(path, payload):
    token = get_token()
    if not token:
        print("No token available, skipping HA post", flush=True, file=sys.stderr)
        return
    try:
        r = requests.post(
            f"{HA_URL}{path}",
...
def handle_heartbeat(j):
    ...
    if panic_active:
        ...
        ha_post("/api/services/meshcore/send_channel_message", {
            "channel_idx": 1,
            "message": "🚨 PANIC BUTTON — Halo — " + j.get("name", "Halo")
        })
...
def handle_event(j):
    ...
    msg = (icon + " " + event + ": " + str(value) +
           " (Schwelle: " + str(threshold) + ") | " +
           "🌡" + str(temp) + "°C CO2:" + str(co2) +
           " AQI:" + str(aqi) + " [" + j.get("time", "?") + "]")
    ...
    if event == "Panic_Button":
        ha_post("/api/services/input_boolean/turn_on", {"entity_id": "input_boolean.halo_panic"})
    # Set sensor value for display — use event value, reset after 30s
    vape_events = ["Vape", "THC", "Smoking"]
    if event in vape_events:
        try:
            vval = max(0.0, min(100.0, float(str(value).replace("?",""))))
        except (ValueError, TypeError):
            vval = 50.0
        ha_post("/api/services/input_number/set_value", {"entity_id": "input_number.halo_vape", "value": vval})
        import threading
        def reset_vape():
            import time; time.sleep(30)
            ha_post("/api/services/input_number/set_value", {"entity_id": "input_number.halo_vape", "value": 0})
        threading.Thread(target=reset_vape, daemon=True).start()
...
class HaloTCPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        print("TCP from " + str(self.client_address[0]) + ": " + msg[:100], flush=True)
        try:
            j = json.loads(msg)
        except Exception as e:
